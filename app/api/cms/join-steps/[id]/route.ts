...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.joinStep.findUnique({
    where: { id },
    include: { media: true },
  });
  ...
  const row = await prisma.$transaction(async (tx) => {
    if (d.imageUrl !== undefined || d.imagePublicId !== undefined || d.imageAlt !== undefined) {
      if (existing.mediaId) {
        ...
        await tx.joinStep.update({ where: { id }, data: { mediaId: m.id } });
      }
    }

    return tx.joinStep.update({
      where: { id },
      data: {
        ...(d.sortOrder !== undefined ? { sortOrder: d.sortOrder } : {}),
        ...(d.published !== undefined ? { published: d.published } : {}),
        ...(d.stepKey !== undefined ? { stepKey: d.stepKey } : {}),
...
export async function DELETE(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.joinStep.delete({ where: { id } });
  if (mediaId) {
    try {
