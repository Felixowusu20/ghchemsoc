...
export default async function NewsPage() {
  ...
  const list = items.filter((n) => n.media);
    ...
    <>
      ...
      <main className="min-h-screen bg-gcs-muted-bg/40 pb-24 pt-28 md:pb-32 md:pt-32">
        <div className="mx-auto max-w-[1440px] px-4 sm:px-6 md:px-12">
          ...
          {!featured ? (
            <p className="mt-12 text-gcs-muted-text">No published news with images yet.</p>
          ) : (
            <>
                      ...
                      <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-gcs-border/50 bg-gcs-surface ring-1 ring-gcs-border/15 transition-colors hover:border-gcs-border hover:bg-gcs-muted-bg/20">
                        <Link href={`/news/${post.slug}`} className="flex flex-1 flex-col">
                          ...
                          <div className="flex flex-1 flex-col p-5">
                            <time className="text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-gcs-muted-text" dateTime={post.date.toISOString()}>
                              {fmt(post.date)}
                            </time>
                            <h3 className="mt-3 text-base font-semibold leading-snug tracking-tight text-gcs-foreground">{post.title}</h3>
                            <p className="mt-2 line-clamp-3 text-sm leading-relaxed text-gcs-muted-text">{post.excerpt}</p>
          ...
          <aside className="mt-16 rounded-2xl border border-gcs-border bg-gcs-surface px-6 py-8 md:flex md:items-center md:justify-between md:gap-10 md:px-10 md:py-10">
            <div className="max-w-xl">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gcs-muted-text">Media &amp; submissions</p>
              <p className="mt-3 text-base font-medium text-gcs-foreground md:text-lg">Pitch a story or request an official statement</p>
              <p className="mt-2 text-sm leading-relaxed text-gcs-muted-text">
                Faculty chapters, industry partners, and student reps can route announcements through the secretariat.
              </p>
            </div>
            <Link
              href="/contact"
